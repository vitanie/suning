...
//  Copyright (c) 2015年 kevin. All rights reserved.
//
@interface UZPagesView ()<UIScrollViewDelegate>{
    
    
    UZPagesViewConfig  *_config;
}

@property (strong, nonatomic) IBOutlet UZPagesBarView *pagesBarView;
...
+(instancetype)viewWithOwnerVC:(UIViewController *)ownerVC pageModels:(NSArray *)pageModels config:(UZPagesViewConfig *)config{
    ...
    UZPagesView *pagesView=[[NSBundle mainBundle] loadNibNamed:NSStringFromClass(self) owner:nil options:nil].firstObject;
...
-(void)event{
    ...
    self.pagesBarView.btnActionBlock=^(UZPagesBarBtn *btn,NSUInteger page){
...
-(void)setPageModels:(NSArray *)pageModels{
    ...
    //模型检查
    BOOL res=[UZPageModel modelCheck:pageModels];
...
-(void)handleForPage:(NSUInteger)page{
    dispatch_async(dispatch_get_main_queue(), ^{
        UZPageModel *pageModel=_pageModels[page];
...
-(NSUInteger)findIndex:(UIView *)subView{
    ...
    for (UZPageModel *pageModel in _pageModels) {
...
-(void)setPage:(NSUInteger)page{
    ...
    _page=page;
    // NSLog(@"page:%ld",page);
    [_delegate uzPagesViewSelectedIndex:page];
    //页码传递
...
-(UZPagesViewConfig *)config{
    
    if(_config == nil){
        
        _config = [[UZPagesViewConfig alloc] init];
    }
...
-(void)setConfig:(UZPagesViewConfig *)config{
...
-(void)jumpToPage:(NSUInteger)jumpPage{
    self.page = jumpPage;
    [self scrollViewActionWithPage:jumpPage];
}
