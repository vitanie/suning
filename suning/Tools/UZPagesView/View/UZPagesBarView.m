 ...
 *  选中的按钮
 */
@property (nonatomic,strong) UZPagesBarBtn *selectedBtn;
...
-(void)btnsPrepare{
    ...
    for (NSInteger i=0; i<_pageModels.count; i++) {
        ...
        UZPageModel *pageModel=_pageModels[i];
        
        UZPagesBarBtn *btn=[UZPagesBarBtn buttonWithType:UIButtonTypeCustom];
...
-(void)layoutSubviews{
    ...
    [self.subviews enumerateObjectsUsingBlock:^(UIView *subView, NSUInteger idx, BOOL *stop) {
        ...
        if([subView isKindOfClass:[UZPagesBarBtn class]]){//过滤其他控件
            
            UZPagesBarBtn *btn=(UZPagesBarBtn *)subView;
            ...
            if(index!=0){
                ...
                UZPagesBarBtn *btnPre=_btns[indexPre];
...
-(void)btnClick:(UZPagesBarBtn *)btn{
...
-(void)setSelectedBtn:(UZPagesBarBtn *)selectedBtn{
 ...
 *  根据btn显示正确的lineView的frame
 */
-(void)adjustLineViewFrameWithBtn:(UZPagesBarBtn *)btn isFirstBtn:(BOOL)isFirstBtn{
...
-(void)setPage:(NSUInteger)page{
    ...
    //取出对应按钮
    UZPagesBarBtn *btn=self.btns[page];
