...
#define RIGHT_SPACE 25
#define MIN_SPACING 20
#define TAB_HEIGHT 44
#define LINEBOTTOM_HEIGHT 2
#define LINEBOTTOM_WIDTH 9 //[_titleSizeArray[0] CGSizeValue].width
...
- (void)setTopTabBottomLineColor:(UIColor *)topTabBottomLineColor{
    _lineBottom.backgroundColor = topTabBottomLineColor;
}
...
- (void)layoutSubviews{
    ...
    _font = _font?_font:[UIFont systemFontOfSize:15];
...
- (UIView *)topTabView{
    if (!_topTabView){
        ...
        if (_isTranslucent) {
            ...
            _topTabView.backgroundColor = [UIColor colorWithHexString:@"#2288D4" alpha:1];;
        }
        if (self.leftButton) {
...
- (UIScrollView *)topTabScrollView{
    if (!_topTabScrollView){
        ...
        _lineBottom = [[UIView alloc] initWithFrame:CGRectMake(0, TAB_HEIGHT-7,LINEBOTTOM_WIDTH, LINEBOTTOM_HEIGHT)];
...
- (UIScrollView *)scrollView{
    if (!_scrollView) {
        ...
        _scrollView.contentSize = CGSizeMake(_selfFrame.size.width * _titles.count, _selfFrame.size.height);
