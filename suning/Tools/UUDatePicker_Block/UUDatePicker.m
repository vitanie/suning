...
- (void)drawRect:(CGRect)rect
{
    ...
    self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, kWindowWidth, 216);
    ...
    if (!myPickerView) {
        float x = 40;
        float width = (kWindowWidth - x*2-60)/2;
        float height = 35;
        ...
        for(int i=0;i<2;i++){
            ...
            [button setBackgroundImage:imageWithColor(colorWithHexString(@"#298AD6")) forState:UIControlStateNormal];
            [button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            button.titleLabel.font = [UIFont systemFontOfSize:15];
            [button setTitle:[arr objectAtIndex:i] forState:UIControlStateNormal];
            button.tag = 0x12802+i;
            [button setFrame:CGRectMake(x, 5, width, height)];
            x = CGRectGetMaxX(button.frame)+60;
            [self addSubview:button];
            [button addTarget:self action:@selector(buttonTap:) forControlEvents:UIControlEventTouchUpInside];
        }
        myPickerView = [[UIPickerView alloc]initWithFrame:CGRectMake(0, height+5, self.frame.size.width, self.frame.size.height-height)];
...
{
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(point_x, 120, 20, 20)];
    label.text = name;
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView
{
    ...
    if (self.datePickerStyle == UUDateStyle_YearMonthDay){
        if (isIOS7) {
            ...
            else if (kWindowWidth == 414)
            {
                xArr = @[@"160",@"240",@"315"];
            }
            [self creatValuePointXs:xArr
                      withNames:@[@"年",@"月",@"日"]];
            }
        return 3;
    }
...
{
    NSLog(@"count:%d",[dayArray count]);
    NSLog(@"row  :%d",row);
    NSLog(@"rowT :%d",[dayArray[row] intValue]);
